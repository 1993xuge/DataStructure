...
public class AVLTree<K extends Comparable<K>, V> {
    ...
    private Node add(Node node, K key, V value) {
        ...
        if (balanceFactor > 1 && getBalanceFactor(node.left) >= 0) {
            // 当前节点，由于左边过高而失衡，并且左子树的平衡因子大于等于0，右旋
            return rightRorate(node);
        }

        // LR
        if (balanceFactor > 1 && getBalanceFactor(node.left) < 0) {
            // 首先对当前节点的左子树进行一次左旋，使当前节点变为LL
        ...
        if (balanceFactor < -1 && getBalanceFactor(node.right) <= 0) {
            // 当前节点 由于右边过高而失衡，并且右子树的平衡因子小于等于0，左旋
            return leftRorate(node);
        }

        //RL
        if (balanceFactor < -1 && getBalanceFactor(node.right) > 0) {
            // 首先对当前节点的右子树进行一次右旋，使当前节点变为RR
    ...
    // x.right = y;
    // y.left = T3;
    private Node rightRorate(Node y) {
        Node x = y.left;
        Node T3 = x.right;
    ...
    //      / \
    //     T3 T4
    private Node leftRorate(Node y) {
        Node x = y.right;
        Node T2 = x.left;
    ...
    private Node remove(Node node, K key) {
        ...
        if (balanceFactor > 1 && getBalanceFactor(retNode.left) < 0) {
            // 首先对当前节点的左子树进行一次左旋，使当前节点变为LL
            retNode.left = leftRorate(retNode.left);
            return rightRorate(retNode);
        }
